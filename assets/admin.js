(() => {
  const ADMIN_PASSWORD = "0admin";
  const PIC_API_KEY = "thepsathit_pic_api_key_v1";
  ...
  const store = window.ThepSathitStore;
  const loginPanel = document.querySelector("#loginPanel");
  const dashboard = document.querySelector("#adminDashboard");
  const logoutButton = document.querySelector("#logoutButton");

  const isLoggedIn = () => sessionStorage.getItem("thepsathit_admin") === "true";
  ...
  const uploadToPic = async (apiKey, file) => {
    ...
    // ใช้ CORS Proxy เพื่อหลอกเบราว์เซอร์ให้ส่ง Header X-API-Key ได้เหมือน cURL
  ...
  document.querySelector("#loginForm")?.addEventListener("submit", async (event) => {
    ...
    sessionStorage.setItem("thepsathit_admin", "true");
    await showDashboard();
  });

  logoutButton?.addEventListener("click", () => {
    sessionStorage.removeItem("thepsathit_admin");
    showLogin();
  });
